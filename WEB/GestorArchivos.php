...
function subirImagen($archivo, $dir, $conexion) {
    ...
    if (($_FILES[$archivo]["type"] == "image/jpeg") || ($_FILES[$archivo]["type"] == "image/png")) {
        ...
        if ($_FILES[$archivo]["error"]) {
            echo '<div class="error">Error ' . $_FILES["archivo"]["error"] . 
                    'al intentar subir el archivo ' . $_FILES[$archivo]["name"] . '</div>';
        } else {
            ...
            if (file_exists("../imagenes/" . $dir . "/" . $_FILES[$archivo]["name"])) {
                echo '<div class="error">Ya hay un archivo con nombre ' . $_FILES[$archivo]["name"] . 
                        '. Renombralo y vuelve a subirlo.</div>';
            } else {
                // Subimos la imagen.
                move_uploaded_file($_FILES[$archivo]["tmp_name"], "../imagenes/" . $dir . "/" . 
                        $_FILES[$archivo]["name"]);
                ...
                $consulta = sprintf("INSERT INTO galeriaimagenes (archivo, directorio) VALUES ('%s','%s')", 
                        $conexion->real_escape_string($_FILES[$archivo]["name"]), $conexion->real_escape_string($dir)
                );
